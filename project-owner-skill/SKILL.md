...
选择主决策时，优先看：**业务影响、架构后果、真实 Trade-off、可验证性、能否扛追问**。不要因为某个技术“新”或“高级”就把它放进主线。
...
不要按 `Redis → Kafka → MySQL → K8s` 的顺序，也不要按“先做了 A、然后做 B、后来又做 C”的开发时间线介绍项目。
...
重要决策通常比较 2–3 个可信方案。比较的是**当前约束下的适配度**，不是抽象地判断“哪个技术更高级”。不要为了凑 A/B/C 制造稻草人方案。
...
这些标签是**准备阶段的事实元数据，不是面试台词**。最终口述自然转换为：

```text
真实经历：我当时负责 / 我实际落地的是……
合理设计：如果进一步生产化，我会……
待验证：这个结论需要通过……压测 / 评测来确认。
```
...
口述必须区分**团队背景**和**个人 Ownership**：可以讲完整系统，但必须明确“团队做了什么”和“我负责决策/落地的是什么”。
